...
func (m UDMProgressModel) renderProgressView() string {
	...
	headerLine := fmt.Sprintf("filename :: %s            Size:: %s",
		...
		sizeStyle.Render(formatBytes(m.tracker.TotalBytes)),
	)
	...
	detailsLine := fmt.Sprintf("completed : %s / %s      Speed :: %s   ETA:: %s",
		formatBytes(m.tracker.BytesCompleted),
		formatBytes(m.tracker.TotalBytes),
		speedStyle.Render(formatSpeed(m.tracker.SpeedBps)),
		etaStyle.Render(formatDuration(m.tracker.ETA)),
	)
...
%s`,
		...
		timeStyle.Render(formatDuration(elapsed)),
		speedStyle.Render(formatSpeed(avgSpeed)),
		border,
	)
...
// UpdateProgress updates the progress tracker (to be called from download functions)
func (m *UDMProgressModel) UpdateProgress(tracker *UDMProgressTracker) {
	*m.tracker = *tracker
}
